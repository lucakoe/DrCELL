...
allFiles = ['\\Naskampa\lts\\2P_PuffyPenguin\\2P_PuffyPenguin\\Umap_Analysis\\UMAPmatrixAllTasks_2532_Select.mat']
outputPath = r'\\Naskampa\lts\2P_PuffyPenguin\2P_PuffyPenguin\Umap_Analysis'
...
for cFile in allFiles:
    ...
    fig1, ax1 = plt.subplots()
    # cColor = sns.color_palette("Dark2", np.max([groups])+1);
    cColor = sns.color_palette("Spectral", np.max([groups])+1);
    ax1.scatter(tempUmapOut[cIdx, 0], tempUmapOut[cIdx, 1], c=[cColor[x] for x in groups[cIdx]], s=2)
    #plt.axis('square')
    ax1.set_aspect('equal', adjustable = 'datalim')
    plt.show() 
    ...
    umap_data = np.column_stack((tempUmapOut, groups))
